...
func (p *Prompt) Render() string {
	...
	templateSource = templateSource + "{{color 21 8}} {{.Shell.PromptEscapeCurentWorkingDirectory}} {{if .Git.IsAGitRepo}}{{color 8 19}}{{else}}{{color 8 0}}{{end}}{{reset}}"
	templateSource = templateSource + "{{if .Git.IsAGitRepo}}{{color 21 19}}  {{.Git.Changes}} {{.Git.Branch}} {{color 19 0}}{{reset}}{{end}}"
